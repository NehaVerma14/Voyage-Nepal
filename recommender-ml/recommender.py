...
def script(place_name, user_age, user_coordinates_x, user_coordinates_y):
    ...
    # USER INPUTS
    gps = (user_coordinates_x, user_coordinates_y)
    place_user_likes = place_name
    user_age = user_age
    ...
    temp1 = list(zip(dist, place))
    popularity = sorted(temp1, key=lambda l: float(l[0]))
    final_popularity = popularity[:5]
    # print(popularity[:5])
    ...
    for x in sorted_similar_places:
        ...
        recommend.append(get_place_from_index(x[0]))
        i = i+1
        if i > 4:
            break
    # print(recommend)
    return final_popularity, recommend
