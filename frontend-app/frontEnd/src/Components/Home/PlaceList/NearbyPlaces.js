...
//   console.log('location', currentLocation);
  ...
  useEffect(() => {
    if (currentLocation) {
      var config = {
        method: 'get',
        url: `/places/nearby?origin=${currentLocation.latitude}%2C${currentLocation.longitude}`,
        headers: {
  ...
  return (
    <SafeAreaView
      style={{paddingTop: 10, backgroundColor: 'white', marginBottom: 50}}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View>
          {nearby.length !== 0 ? (
            <FlatList
              ...
              renderItem={({item}) => (
                <Pressable onPress={() => getPlacesById(item.place._id)}>
        ...
        <View>
          <View>
            <Text style={styles.recommendText}>Top Rated Place</Text>
          </View>
          <View>
            {topRated.length !== 0 ? (
              <FlatList
                ...
                renderItem={({item}) => (
                  <Pressable onPress={() => getPlacesById(item.place._id)}>
                    <Card place={item.place} key={item.place._id} />
                  </Pressable>
                )}
              />
            ) : (
              <Text style={{color: Colors.warning, marginLeft: 25}}>
                Sorry, No Top rated places found in Database.
              </Text>
            )}
          </View>
        </View>
        ...
        <View>
          <View>
            <Text style={styles.recommendText}>Most Visited Places</Text>
          </View>
          <View>
            {mostVisited.length !== 0 ? (
              ...
              <Text style={{color: Colors.warning, marginLeft: 25}}>
                Sorry, No Most Visited places found in Database.
              </Text>
            )}
          </View>
        </View>
